...
### 对象池模式在 Unity 场景中的价值
- 典型场景：子弹、特效、怪物生成等高频创建/销毁对象，减少 GC 和 Instantiate/Destroy 开销。
- 关键点：池化对象应实现 `OnEnable/OnDisable` 重置状态；注意回收到池时解除事件订阅、协程、Tween，避免悬挂引用。
- 线程安全：大多情况下池运行在主线程；若结合 Job/Burst，仅存储数据结构，主线程负责实例化/回收。

### 观察者模式 vs C# 事件/UnityEvent
- C# 事件/委托是观察者的语言级实现，`event` 可限制外部只能订阅/取消，不能直接触发。
- UnityEvent 适合编辑器可视化绑定，但有装箱和反射开销；高频逻辑优先使用委托或自建事件总线。
- 需注意取消订阅时机，避免对象销毁后仍有回调引用导致内存泄漏。

### 命令模式在输入/技能系统中的应用
- 将一次操作封装为命令对象（含 `Execute/Undo`），解耦输入采集与业务逻辑；可实现撤销、录制与回放（如关卡回放、AI 训练）。
- 在技能系统中，一个技能可拆分为多条命令（扣资源、播动画、施加伤害、生成特效），支持排队、打断、回滚。
- 与状态机结合：命令进入队列前检查当前状态是否可执行；执行失败时返回原因，便于 UI 提示。
