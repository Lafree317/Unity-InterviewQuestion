...
### 设计 LRU 缓存（常见手撕题）
- 要求 `O(1)` 的 `Get/Put`，用**哈希表 + 双向链表**：哈希表定位节点，链表维护最近使用顺序，尾部最老。
- `Get`：存在则移动到链表头并返回值；不存在返回 -1。
- `Put`：存在则更新值并移到头；不存在则在头插入新节点，容量超限时移除尾节点并从哈希表删除。

### 二叉树遍历与常见考点
- 递归模板：前序(根左右)、中序(左根右)、后序(左右根)。
- 迭代写法：借助栈，前序可模拟递归，后序可用“根右左”再反转；层序遍历用队列。
- 变种：判断平衡树（递归返回高度或 -1）、最近公共祖先（分治返回命中的节点）。

### 拓扑排序的应用
- 用于有向无环图的依赖调度（资源加载、任务编排、编译依赖）。
- 常用算法：Kahn（入度为 0 入队，逐步删除边）或 DFS 逆后序；检测环：结果数量 < 节点数即存在环。
